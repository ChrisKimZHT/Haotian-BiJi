...
从流程就可以看出 GKD 就是针对其他蒸馏方案的缺点设计的。首先模型训练的数据不再是固定的 $(x,y)\in(X,Y)$，而是固定的 $x\in X$ 后由**学生模型动态采样**生成得到的 $y$。其次，学生模型不再去拟合他人（来自标注或教师模型）生成的序列，学习如何在自己的序列前缀上让概率分布与教师模型相似。这样在训练和推理时，学生模型见到的序列是一致的，均是自己生成的序列。
