...
如果要让区间修改既支持加法，也支持乘法，那么首先肯定需要两种修改标记，因为加法和乘法的逻辑是不同的。我们用 $\text{add\_val}$ 记录加法标记，$\text{mul\_val}$ 来记录乘法标记。在初始化时，$\text{add\_val}_i=0,\; \text{mul\_val}_i=1$. 综上，建树代码为：
...
（实际上上传操作也能单独提取出来，不过这里因为只有一行上传操作，所以就不单独写了）
